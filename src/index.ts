...
// White Page
app.get('/', function (req: express.Request, res: express.Response): void /*use VOID when there is no return*/ {
    res.send(`
        <body style="margin: 0;">
...
app.get('/:color', function (req: express.Request, res: express.Response): void {
    let myColor: unknown /*when the query parameter is unknown*/ = req.params.color
    if (typeof(myColor) === 'string'){ /*to fix the error message, if typof is 'unknown' then error will be thrown*/
        res.send(`
        <body style="margin: 0;">
